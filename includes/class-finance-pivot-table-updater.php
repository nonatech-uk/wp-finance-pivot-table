<?php
/**
 * GitHub Plugin Updater for Finance Pivot Table
 *
...
class Finance_Pivot_Table_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-finance-pivot-table')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'finance_pivot_table_github_update_' . md5($repo);
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) array(
            ...
            'sections' => array(
                'description' => 'Interactive pivot table for displaying parish council financial data. Features year tabs, drill-down hierarchy, summary statistics, and CSV download.',
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?finance_pivot_check_update=1'), 'finance_pivot_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['finance_pivot_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'finance_pivot_check_update')) {
            return;
        }
